...
sudo docker exec demootrs_otrs sed -i 's/RUNTO/Step 4/' /opt/otrs/Kernel/Config/Files/XML/DemoOTRS.xml 
sudo docker exec demootrs_otrs sed -i 's/TOLINK/load%20step%2004/' /opt/otrs/Kernel/Config/Files/XML/DemoOTRS.xml
sudo docker exec demootrs_otrs sed -i 's/HEREIAM/Step 3/' /opt/otrs/Kernel/Config/Files/XML/DemoOTRS.xml #current step
...
sudo docker exec demootrs_otrs cp /opt/otrs/Kernel/scripts/s3/*.pm /opt/otrs/Kernel/Config.pm

#add faq article
sudo docker exec demootrs_otrs su - otrs -c 'perl /opt/otrs/bin/otrs.Console.pl Admin::FAQ::Import "/opt/otrs/Kernel/System/Console/Command/Custom/Admin/FAQ/c01s03.csv"'
...
sudo docker exec demootrs_otrs su - otrs -c '/opt/otrs/bin/otrs.Console.pl Custom::Admin::ACL::Add --file ACL_Hide_Unclassified.yml'
...
#add queues
sudo docker exec demootrs_otrs su - otrs -c 'perl /opt/otrs/bin/otrs.Console.pl Custom::Admin::Queue::Update --name "Incidents 1st Level" --group "users" --system-address-name "support@firma.de" --ID "6"'
...
sudo docker exec demootrs_otrs su - otrs -c 'perl /opt/otrs/bin/otrs.Console.pl Custom::Admin::Queue::Update --name "Service Requests 1st Level" --group "users" --system-address-name "support@firma.de" --ID "7"'
...
#add generic agent
sudo docker exec demootrs_otrs su - otrs -c '/opt/otrs/bin/otrs.Console.pl Custom::Admin::GenericAgent::Add --name "Move Incidents" --file /incidents.yml'
sudo docker exec demootrs_otrs su - otrs -c '/opt/otrs/bin/otrs.Console.pl Custom::Admin::GenericAgent::Add --name "Move Service Requests" --file /servicerequests.yml'
